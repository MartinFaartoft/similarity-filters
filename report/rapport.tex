...
Given a set $S$ of binary vectors (e.g. 32-bit words) create a data structure that given a query vector x determines if there exists a vector S that is a factor of \textit{k} close to x.
...
In this report we will analyze and compare two different approaches of solving the problem of querying a set for an approximate match on a query element.
%     Bloom filters
%     LSH
%     k-closeness
%     naiv løsning
