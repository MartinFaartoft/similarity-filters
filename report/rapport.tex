...
While keeping in mind that standard Bloom filters only answer exact membership, we see that one way of achieving this is by relying on extra insertions. Every time an element is added to the Bloom filter, all elements within distance $d$ are generated and added in addition to the original element.
...
In their experiments they manage to achieve false postive rates and false negative rates of 1.5\% and 0.2\% respectively for 1000 elements, with $\epsilon = 0.1$ and $\delta = 0.4$. For 10000 they achieve  0.016\% false positive rate and 0.003\% false negative rate, with with $\epsilon = 0.05$ and $\delta = 0.4$. For 1000 elements they use 64\% of the total space of the elements, and for 10000 elements they use 51\% of the total space. \todo{Kan I ikke se på de resultater også. Jeg synes ikke det giver mening}
...
\todo{flyt til starten af næste section}
...
This requires $2^{|W|}$ queries in the worst case, and half \todo{det med half gælder kun hvis vi ved at den er close}of that in the average case, assuming we 'short-curcuit' (stop querying) after receiving the first 'close' response.
...
In this paper we have implemented the Distance Sensitive Bloom Filter\cite{paper:harvard} presented by Kirsch and Mitzenmacher. 
We have been able to reproduce some of their experiments.
...
Finally we have introduced an extension to the DSBF that allows Wildcard search. The method sacrifices the asymptotic constant query time guarantee of the DSBF, however in practical cases\todo{hmm}
